...
impl VmSpace {
    ...
    pub fn allocate_virtual_address_range(&self, len: usize) -> Result<Range<Vaddr>> {
        ...
        let preempt_guard = disable_preempt();
        let mut cursor = self.cursor(&preempt_guard, &user_space_range)?;
...
impl<'vmspace, G: AsAtomicModeGuard> CursorMut<'_, '_, 'vmspace, G> {
    ...
    /// This method will bring the cursor to the next slot after the modification.
    ///
    /// FIXME: Add a check to ensure the IO memory is not already mapped.
    pub fn map_iomem(&mut self, io_mem: IoMem, prop: PageProperty) {
        // Check whether the range within is checked
        // Handle it correspondingly
        // Check the alignment of the range within
        let mut current_paddr = io_mem.paddr();
        let io_mem_end = io_mem.paddr() + io_mem.length();
        while current_paddr < io_mem_end {
        ...
        if iomems
            .iter()
            .find(|iomem| iomem.paddr() == io_mem.paddr() && iomem.length() == io_mem.length())
            .is_none()
        {
            iomems.push(io_mem);
        }
