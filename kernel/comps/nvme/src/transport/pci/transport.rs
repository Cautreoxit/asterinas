...
impl NvmePciTransport {
    ...
    ///
    /// Returns `Err` with the device if MSI-X setup fails.
    ...
    fn init_msix(common_device: &PciCommonDevice) -> Option<NvmeMsixManager> {
        ...
        for cap in capabilities.iter() {
            if let CapabilityData::Msix(msix_data) = cap.capability_data() {
                ...
                let Some(manager) = NvmeMsixManager::new(msix_data_clone) else {
                    warn!("[NVMe]: IRQ line allocation failed");
                    return None;
                };
                info!(
...
impl DbregAccess<'_> {
    /// Writes a queue doorbell; the caller must hold the queue lock so updates stay ordered.
